...
**16. Correction to #14 — brand recognizability is a hard requirement for Marketing Lessons, not optional (user feedback, batch 7): Luckin Coffee and Ocean Spray were real and shocking but not household names, so the hook had less pull than it should have.** The name itself needs to be doing work in the hook — a viewer should recognize the brand instantly and lean in *because* they already have a relationship with it. Pick from genuinely universal names (Amul, Netflix, Coca-Cola, Amazon, McDonald's, Zomato/Swiggy, Cadbury, Nike — brands basically everyone in the target audience already uses or knows) and *then* apply the shock-value bar from #14 within that shortlist — find the surprising mechanism inside a brand everyone already knows, don't trade recognizability away for a more obscure "wow" story. Both matter; recognizability is now the non-negotiable filter, shock value is what you optimize for after that.
